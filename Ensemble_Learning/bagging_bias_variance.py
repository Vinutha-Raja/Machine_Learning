...
def calculate_bias(iter_pred, labels):
...
def repeated_bagging(num_iter):
    ...
    for i in range(num_iter):
        ...
        training_samples = data_df.sample(frac=0.1, replace=False, random_state=i)
        bagging_algorithm(100, training_samples, test_data_df, first_iter_pred,
                                                            last_iter_pred, i)
    ...
    first_bias = calculate_bias(first_iter_pred, labels)
    last_bias = calculate_bias(last_iter_pred, labels)
    ...
    print("single tree: bias : {}, variance: {}".format(first_bias.mean(axis=1), first_variance.mean(axis=1)))
    print("bagged tree: bias : {}, variance: {}".format(last_bias.mean(axis=1), last_variance.mean(axis=1)))
