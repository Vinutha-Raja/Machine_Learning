...
def perceptron_algorithm(epoch, data, learning_rate):
    ...
    for i in range(epoch):
        ...
        for row_num in range(len(np_data)):
            y_i = 1 if np_data[row_num][5] == 1 else -1
...
def predict_test_data(data, a):
    pred_list = []
    labels = []
    for row in data:
        y_i = 1 if row[5] == 1 else -1
        labels.append(y_i)
        pred = vector_prod(a, row[0:5])
        if pred < 0:
            pred_list.append(-1)
        else:
            pred_list.append(1)
...
print(training_data)
final_weight, a = perceptron_algorithm(10, training_data, 0.1)
print("final_weight_vector: ", final_weight)
print("a:", a)
test_data = read_data('./bank-note/test.csv')
predictions, labels = predict_test_data(test_data, a)
# print(predictions)
